...
win <- c()
for (i in 1:nrow(game_score)){
  win <- c(win, game_score[i,1] > game_score [i,2])
}
print(win)
```
...
```{r}
game_score <- cbind(game_score, win) #turns the win logicals to 1 or 0 since matrices take a single data type
print(game_score)
```
